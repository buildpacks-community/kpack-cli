...
func testWaitForImage(t *testing.T, when spec.G, it spec.S) {
	var (
		...
		successfulBuild = &v1alpha2.Build{
	...
	it.Before(func() {
		...
		clientset.Resources = []*metav1.APIResourceList{{GroupVersion: "kpack.io/v1alpha2"}, {GroupVersion: "kpack.io/v1alpha1"}}
	...
	when("the image is already in a successful state", func() {
		it("returns the already built image and tails the finished logs", func() {
			image.Status = v1alpha2.ImageStatus{
